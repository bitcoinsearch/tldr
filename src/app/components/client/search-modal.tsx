...
import { SearchTotalHits, SearchResponseBody } from "@elastic/elasticsearch/lib/api/types";
...
const SearchBox = () => {
  ...
  const [searchQuery, setSearchQuery] = React.useState<SearchDataParams | null>(null);
  ...
  if (data) {
    ...
    const domainAggregations = data?.pages[pageLength - 1].aggregations?.domain as unknown as any;
    const totalHitsAsSearchTotalHits = data?.pages[pageLength - 1].hits?.total as SearchTotalHits;
    const bitcoinDevCount = domainAggregations?.buckets.find((url: string) => url === urlMapping[BITCOINDEV])?.doc_count ?? 0;
    const lightningDevCount = domainAggregations?.buckets.find((url: string) => url === urlMapping[LIGHTNINGDEV])?.doc_count ?? 0;
  ...
  const showDescription = !isFetching && !isFetchingNextPage && !isError && Boolean(results?.totalSearchResults && results?.totalSearchResults > 0);
      ...
      <Dialog.Root open={open} onOpenChange={setOpen}>
            ...
            {isMobile ? null : (
              <div className='w-[411px] h-[49px] border border-gray-custom-400 rounded-full flex items-center justify-between'>
                ...
                <input
                  ...
                  readOnly
                />
                <button className='bg-orange-custom-100 py-[8.5px] px-4 rounded-full absolute right-1'>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content
            ...
            } p-4 bg-white shadow-xl z-20 ${open ? "animate-slideIn" : "animate-slideOut"}`}
            style={{
              top: isMobile ? "50%" : `calc(${inputContainerRef.current?.getBoundingClientRect().bottom}px + 10px)`,
          ...
          >
            <Dialog.Title className='m-0 text-[16px] font-medium flex items-center gap-x-4'>
              ...
              <span className='flex items-center gap-x-2'>
                <button
                  className={`border rounded py-1 px-2 text-xs hover:bg-slate-100 ${searchQuery?.path === BITCOINDEV && "bg-slate-200"}`}
                  onClick={() => {
                ...
                <button
                  className={`border rounded py-1 px-2 text-xs hover:bg-slate-100 ${searchQuery?.path === LIGHTNINGDEV && "bg-slate-200"}`}
                  onClick={() => {
            ...
            <Dialog.Description
              className={`flex justify-between mt-[10px] mb-5 text-[15px] leading-normal ${showDescription ? "visible" : "hidden"}`}
            >
              <div>
                Search results for {searchQuery?.query.keyword && <span className='font-medium italic'>{searchQuery.query.keyword}</span>}
                {searchQuery?.query.author && (
                  <span>
                    {" "}
                    {searchQuery?.query.keyword && "by"} <span className='font-medium italic'>{searchQuery.query.author}</span>
                  </span>
                )}
              </div>
            </Dialog.Description>
